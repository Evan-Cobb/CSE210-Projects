...
public static class RuleWorkshop
{
    public static RulePack BuildInitialPack(IReadOnlyList<string> folderNames)
    {
        RulePack pack = new RulePack();
        RunInitialWorkshop(pack, folderNames);
        return pack;
    }

    private static void RunInitialWorkshop(RulePack pack, IReadOnlyList<string> folderNames)
    {
        if (pack == null)
        {
        ...
        IReadOnlyList<string> destinations = BuildDestinations(folderNames);
        if (destinations.Count == 0)
        {
            throw new InvalidOperationException("No destination folders are available.");
        }
        ...
        while (true)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("Rule Workshop:");
    ...
    public static void ReorderDuringChallenge(RulePack pack)
    {
        if (pack == null)
        {
            throw new ArgumentNullException(nameof(pack));
        ...
        Console.WriteLine("Priority Change:");
        ReorderRules(pack);
    }

    public static void AddRuleDuringChallenge(RulePack pack, IReadOnlyList<string> folderNames)
    {
        if (pack == null)
        {
            throw new ArgumentNullException(nameof(pack));
    ...
    private static IReadOnlyList<string> BuildDestinations(IReadOnlyList<string> folderNames)
    {
    ...
    private static void AddRule(RulePack pack, IReadOnlyList<string> destinations)
    {
        Console.WriteLine();
